...
const char *WINDOWTITLE = { "Particle System -- Robert Hudspeth" };
...
const float RMIN = { 30.f };
const float RMAX = { 40.f };
...
void InitGraphics(void) {
	...
	for (i = 0, p = Fire; i < NUMFIRES; i++, p++) {
		...
		double XRotY = (cos(th2) + sin(th2));
		double ZRotY = (cos(th2) - sin(th2));
		...
		float gray = Ranf(0.7f, 1.f);
		p->r = Ranf(gray - 0.05f, gray);
		p->g = Ranf(gray - 0.05f, gray);
		p->b = Ranf(gray - 0.05f, gray);
		p->ti = 10;
	}
